...
import { X, Phone, Mail, MapPin, Building2, Layers, ShieldCheck, User } from "lucide-react";
...
export default function StructureMemberModal({ member, onClose }: StructureMemberModalProps) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col transition-all transform scale-100 animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Top Header / Image Area */}
        <div className="relative w-full aspect-[16/9] sm:aspect-[2/1] bg-gradient-to-br from-emerald-950 via-emerald-900 to-green-950 flex items-center justify-center overflow-hidden shrink-0">
          ...
          {/* Level Tag Overlay */}
          <div className="absolute bottom-3 left-4 bg-emerald-900/90 backdrop-blur-md text-emerald-100 px-3.5 py-1.5 rounded-full text-xs font-semibold shadow-md border border-emerald-500/30 flex items-center gap-1.5">
            {member.level === "STATE" ? (
          ...
          <button
            onClick={onClose}
            type="button"
            className="absolute top-4 right-4 p-2.5 rounded-full bg-black/50 hover:bg-black/80 text-white transition-all backdrop-blur-md border border-white/20 shadow-lg group focus:outline-none focus:ring-2 focus:ring-emerald-500"
